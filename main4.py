...
async def chat(message: str = Form(...), file: UploadFile = File(...)):
    ...
    Compare the data like who is the best customer based on having details.
    Predict the income.
    """
    ...
    while attempt < max_attempts:
        try:
            ...
            logger.error(f"Resource exhausted error: {e}. Retrying in 30 seconds...")
            time.sleep(30)  # Wait before retrying
            attempt += 1
        except AttributeError as e:
            logger.error(f"Error extracting response text: {e}")
            return {"response": "There was an error processing the response from the model."}
        except Exception as e:
            logger.error(f"Unexpected error: {e}")
            return {"response": "An unexpected error occurred. Please try again later."}

    return {"response": "Unable to process the request after several attempts. Please try again later."}
